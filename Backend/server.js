    ...
    db.query(vendorFormSql, vendorFormValues, (err, vendorFormResult) => {
      ...
      if (selectedRequirements.length > 0) {
        ...
        db.query(requirementsSql, [requirementsValues], (err) => {
          if (err) {
            console.error(
              "Error inserting requirements into the database:",
              err
            );
            return res.status(500).json({
              error: "Error inserting requirements into the database",
            });
          }
        });
      }
      ...
      ? [
          ...
          ...selectedServices.map((service) => ({
            price_data: {
              ...
              unit_amount:
                frontendPrices[service.service][selectedPackage] * 100,
            },
            quantity: 1,
          })),
        ]
